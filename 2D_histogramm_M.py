...
def hist(r_par, mu_par, beta_par, fc_par):
    treshold = [0,2,4,6,8,10]
    fc_final= [0,100,200,300,400,500,600,700,800,900,1000]
    hist_mat = np.zeros((10, len(treshold)))
    for i in range(len(treshold)):
       hist_mat[:,i]= creation_column(r_par, mu_par, beta_par,fc_par,treshold[i])
    #print hist_mat
    
    if not os.path.exists("experiments/r=%02d_mu=%.2f_Beta=%.1f_fc=%.2f" % (r_par,mu_par, beta_par, fc_par)):
        os.makedirs("experiments/r=%02d_mu=%.2f_Beta=%.1f_fc=%.2f" % (r_par,mu_par, beta_par, fc_par))
    filename="experiments/r=%02d_mu=%.2f_Beta=%.1f_fc=%.2f/heatmap" % (r_par,mu_par, beta_par, fc_par)
    np.save(filename, hist_mat)

    return

def plot_hist(r_par, mu_par, beta_par, fc_par):

    filename="experiments/r=%02d_mu=%.2f_Beta=%.1f_fc=%.2f/heatmap.npy" % (r_par,mu_par, beta_par,fc_par)
    ...
    axes=plt.gca()
    axes.xaxis.set_ticklabels(['','0', '2', '4', '6', '8', '10'])
    ...
    plt.title("r="+str(r_par)+" mu="+str(mu_par)+" Beta="+str(beta_par)+" fc="+str(fc_par))
    plt.colorbar()
    plt.show()
...
#           for fc_par in [0.00,0.10,0.30,0.50,0.70,0.90,1.00]:
#              hist(r_par, mu_par, beta_par, fc_par)

#hist(8,0.00,10.0,0.90)
plot_hist(8,0.00,10.0,0.50)
