...
# calculation of all the transitions probabilities with respect to strengths
def values_of_fermi_function_with_strength(tab):
    l = len(tab)
...
def complete_game(A):
    ...
    for i in range(number_of_games):
        ...
        for k in range(number_of_groups):
            ...
            for m in range(N):
                b = np.random.choice(N, 1, p=[B[
                                     k*N+l] / sum([B[k*N+p] for p in range(N)]) for l in range(N)])[0]
                # in each group, test if m player will follow the leader
                if (b < following[groups[k*N+m]-1][groups[k*N+n]-1]):
    ...
    # W, tableau des payoffs alimenté chaque épe
    W = W/number_of_games
    return W
