...
def heatmap(r_par, mu_par, beta_imit, beta_follow, fc_par, M_par):
    foldername = "simulations_with_leadership_mutation_strength_with_fermi_function/r=%02d_mu=%.2f_Beta_imit=%.1f_Beta_follow=%.1f_fc=%.2f_M=%02d" % (r_par, mu_par, beta_imit, beta_follow, fc_par, M_par)
    matr=np.zeros((6000, 10))
    matrix=np.zeros((10,6000, 10))
    c=0
    #print(os.listdir(foldername))                                       # creation of the matrix
