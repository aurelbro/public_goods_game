...
def main(A, number_of_rounds):
    ...
    for i in range(1, number_of_rounds):
        ...
        print(i,"c:"+ str(sum(C)/number_of_cooperators(B)),"d:"+str(sum(D)/(len(A)-number_of_cooperators(B))))
        if (B != A):
            A = B
...
for j in range(10):
    plt.plot(np.arange(1, number_of_generations+1), main_4([0]*int(Z*(1-fc)) + [1]*int(Z*fc), number_of_generations))
plt.show()